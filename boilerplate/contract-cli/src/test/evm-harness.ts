/**
 * Local-EVM harness for the vault integration tests.
 *
 * Spins up a Hardhat node, deploys a standard-compliant ERC20 (TestUSDC, 6 decimals), and
 * exposes helpers to fund the MPC-derived EVM addresses (USDC mint + ETH for gas). The MPC
 * simulator then signs and broadcasts real EIP-1559 transfers to this node, so the api
 * tests exercise the genuine cross-chain path instead of hand-crafted MPC responses.
 */
