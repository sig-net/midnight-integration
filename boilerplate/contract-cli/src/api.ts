...
// Build + sync a wallet but DO NOT block waiting for funds. Used for a second
// wallet (e.g. the e2e bearer-transfer recipient) that the dev chain's genesis
// does not endow — it is funded programmatically from the genesis wallet instead.
...
/**
 * Fund a freshly-built wallet (one the dev chain's genesis does not endow) so it can
 * pay its own proving fees. Fees on Midnight are paid in DUST, which is generated by
 * holding NIGHT — a brand-new wallet has neither. Flow (per the wallet-developer-guide,
 * verified against the standalone):
 *   1. the funder transfers NIGHT to the target's unshielded address (funder pays the fee);
 *   2. the target registers its received NIGHT for dust generation —
 *      register → finalizeRecipe → submit. The signature is supplied via the signData
 *      callback passed INTO registerNightUtxosForDustGeneration; do NOT also call
 *      signRecipe (double-signing makes the node reject it with "Custom error: 192");
 *   3. the NIGHT's own already-generated dust pays the registration fee (finalizeRecipe
 *      balances it), and once processed it becomes the target's spendable fee balance.
 * Returns the target's resulting dust balance.
 */
...
/**
 * Submit completeWithdraw, optionally supplying coin-public-key → encryption-public-key
 * mappings. The refund path mints a shielded coin to the stored refundPk; if that key
 * belongs to ANOTHER wallet, the prover needs that wallet's encryption public key to
 * build the Zswap output (a coin public key alone can't receive a shielded coin). The
 * mapping values/keys are normalized internally, so pass each wallet's
 * coinPublicKey.toHexString() → encryptionPublicKey.toHexString().
 */
