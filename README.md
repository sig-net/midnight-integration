...
- **4.** The MPC network observes execution of the signed transaction on the foreign blockchain and posts an attestation thereof back to Midnight.
  ...
  - The serialised output it attests depends on whether that execution succeeded:
    - **Foreign transaction success:** the MPC extracts the output of the transaction execution and serialises it according to the output serialisation schema given in the **SignBidirectionalEvent** it reacted to in step **2.**, applying the native Midnight standard library serialisation protocol.
    - **Foreign transaction failure:** there is no output to serialise, so the serialised output is instead a fixed 5-byte MPC failure payload `deadbeef01` (see [Handling Failure](#handling-failure)).
  - From here the two branches converge: the MPC creates the attestation as the ECDSA signature over the attestation digest `upgradeFromTransient(transientHash([requestId, serializedOutput]))` (see [`calculateSignetAttestationDigest`](./packages/signet-midnight/src/Signet.compact#L311)) of whichever serialised output the branch produced, signed with the integrating contract's own **Response Signing Key** specific to the contract (see [Derived keys](#derived-keys)).
...
- **5.** The integrating dApp collects the execution output and its attestation and submits both back to the integrating contract, completing the cross chain interaction.
  ...
  - It then reconstructs the exact serialised output the MPC attested, mirroring step **4.**'s branch:
    - **Foreign transaction success:** the dApp obtains the actual execution output off chain (see the output recovery note below: it broadcast the transaction in step **3.**, so it can read the result) and serialises it exactly as the MPC did in step **4.**, against the same schema, so the bytes match the attested ones byte for byte.
  ...
  - It submits the attestation and the reconstructed serialised output to a completing circuit on the integrating contract (`completeCrossChain(...)` in the diagram), which recomputes the attestation digest from the output bytes and verifies the MPC's signature in-circuit via [`verifyRespondBidirectionalEvent`](./packages/signet-midnight/src/Signet.compact#L331) against the response key the contract pinned after deploy (see [Derived keys](#derived-keys)). Success and failure verify identically, since step **4.** attests both with the same digest and key.
  - The completing circuit settles by the same distinction: it determines the foreign transaction failed, and reacts accordingly, when the verified output bytes equal the 5-byte failure payload exactly, and it treats them as a success otherwise, deserialising them against its output schema ([`isMpcFailureOutput`](./packages/signet-midnight/src/constants.ts#L38) is the off-chain twin of that check). **Warning:** a contract whose successful serialised output could itself equal the failure payload cannot tell success from failure at all: see [Handling Failure](#handling-failure) for what types of respond schemas are vulnerable and how to protect against it.
...
A failed foreign transaction (i.e. a transaction that reverted on chain, or another transaction consumed its nonce) still completes the flow. The MPC attests **a fixed failure payload** (`deadbeef01`) in step **4.**, the dApp submits that payload in step **5.**, and the integrating contract settles against it in-circuit, i.e. the flow is the same as for a successful foreign transaction execution.

- **The failure payload** is the 5 bytes `deadbeef01`: the magic error marker `0xdeadbeef` followed by one `0x01` byte. It is [`MPC_FAILURE_OUTPUT`](./packages/signet-midnight/src/constants.ts#L29) in this library, and it originates in the MPC node's [`MAGIC_ERROR_PREFIX`](https://github.com/sig-net/mpc/blob/e180584f60c6e44819d0847687589370d2d8d2ee/chain-signatures/node/src/respond_bidirectional.rs#L24) and [`process_failed_tx`](https://github.com/sig-net/mpc/blob/e180584f60c6e44819d0847687589370d2d8d2ee/chain-signatures/node/src/respond_bidirectional.rs#L141). Its width is 5 bytes regardless of the request's respond serialisation schema.
- **The attestation carries no success flag.** Success and failure produce the same construction: the MPC's ECDSA signature over the attestation digest `upgradeFromTransient(transientHash([requestId, serializedOutput]))`, signed with the same **Response Signing Key**. The only difference is that serialisedOutput is exactly `deadbeef01` if the transaction failed instead of some serialised execution output.
- **Settlement must route on the verified bytes.** Contracts must determine whether a foreign transaction failed by checking the verified output equals the failure payload exactly, and treating the output as a success otherwise, deserialising it against its respond schema ([`isMpcFailureOutput`](./packages/signet-midnight/src/constants.ts#L38) is the off-chain twin of that check). The best way to route the two outcomes separately is deliberate use of Compact's fixed-width `Bytes<n>` circuit arguments: ensure the respond schema's packed width is not 5 bytes, then expose two settle circuits, one taking the schema's `Bytes<n>` for success and one taking `Bytes<5>` for failure, asserting exact equality with the payload. Every attested output then type-fits exactly one of the two.

>**Warning:** This identical success and failure flow mean that if the 5 byte `deadbeef01` is a valid successful seralised output the contract will not be able to tell the difference between a success and failure!

### Which contracts are vulnerable

The MPC does not reserve the failure payload. A successful transaction whose output genuinely serialises to `deadbeef01` is attested with exactly those bytes: the success path serialises the real output with no comparison against the payload (see the MPC node's [`process_success_tx`](https://github.com/sig-net/mpc/blob/e180584f60c6e44819d0847687589370d2d8d2ee/chain-signatures/node/src/respond_bidirectional.rs#L175)). Guarding against the resulting ambiguity is the integrating contract's responsibility, through its choice of respond serialisation schema:

- **A schema whose packed width is anything but 5 bytes is fully safe.** The attestation digest covers the output bytes at their full length, so a success attestation and a failure attestation can never verify over each other's output.
- **A schema whose packed width is exactly 5 bytes is vulnerable.** A prefix check is not safe, as a legitimate packed output can begin with `0xdeadbeef`, and byte inspection cannot route settlement at all: the contract would have to recompute both candidate digests (over the recovered output and over the failure payload) and check which one the MPC attested. Even that breaks down in the worst case: a genuine success output equal to `deadbeef01` produces an attestation byte-identical to a failure's, indistinguishable in principle to the contract, the dApp and every observer. A contract that refunds on failure would then refund a transaction that actually executed.

**The rule: never give a request a respond serialisation schema that packs to exactly 5 bytes.** If one is truly unavoidable, either ensure the legitimate output domain excludes `deadbeef01`, or design both settlement branches so that either interpretation of that value is safe (in particular, a refund must not re-mint value the success interpretation considers spent).
