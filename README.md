...
- **The attestation signs the outcome kind.** Success and failure use the same attestation digest formula `upgradeFromTransient(transientHash([requestId, blockHeight, outputKind, serializedOutputLength, serializedOutput]))` and the same **Response Signing Key**, and differ in the `outputKind` the digest commits to and the **RespondBidirectionalEvent** carries, beside the serialised output the signature verifies over.
