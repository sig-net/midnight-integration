...
- **2.** The MPC network, watching for events on the Singleton contract, picks up the emitted Sign Bidirectional Event Notification.
  ...
  - The signature is then made available on Midnight with the MPC calling the `respond` circuit on the **Sig Network Singleton**, emitting a **SignatureResponded Event**.
- **3.** The integrating dApp, watching for events on the Singleton contract, picks up the emitted **SignatureResponded Event**.
...
- **4.** The Network MPC observes foreign blockchain and attests execution
  ...
  - It extracts the output of the transaction execution and serialises it according the the output serialisation schema given in the **SignBidirectionalEvent** it reacted to in step **2.** applying the native Midnight standard library serialisation protocol.
  - The MPC then creates an attestation of the execution output as the ECDSA signature over the attestation digest `keccak256(requestId || serializedOutput)` with the integrating contracts own `Response Signing Key` specific to the contract (see [Derived keys](#derived-keys)).
  - The output attestation then made available on Midnight with the MPC calling the `respondBidrectional` circuit on the **Sig Network Singleton**, emitting a **RespondBirectional Event**.
