...
- **2.** The MPC network, watching for events from the Singleton contract, picks up the emitted Sign Bidirectional Event Notification.
  ...
  - The signature is then made available on Midnight when the MPC calls the `respond` circuit on the **Sig Network Singleton**, emitting a SignatureResponded Event.
