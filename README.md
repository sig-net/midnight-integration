...
| [`packages/midnight-serde-rs`](packages/midnight-serde-rs) | `signet-midnight-serde` (crate, unpublished) | Rust twin of the same serde byte layout: zero runtime dependencies, tested purely against the committed conformance corpus (`cargo test` needs no Node or compactc) |
