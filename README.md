...
| ------- | ------ | ------ |
| `@sig-net/*` npm packages | 0.20.0-rc.1 | [`packages/*/package.json`](packages) |
| fakenet MPC responder | `ghcr.io/sig-net/fakenet:0.15.0` | [`docker-compose.yaml`](docker-compose.yaml) |
...
| [`packages/midnight-serde-ts`](packages/midnight-serde-ts) | `@sig-net/midnight-serde` | TypeScript twin of Compact's builtin `serialize<T,N>`/`deserialize<T,N>` byte layout, pinned byte-for-byte against compiled fixture circuits. Zero runtime dependencies |
| [`packages/midnight-serde-conformance`](packages/midnight-serde-conformance) | repo-private | Conformance kit for the serde byte layout: the fixture contract, shared descriptor tables, corpus generator and the committed golden corpus every serde implementation is pinned against |
| [`packages/midnight-serde-rs`](packages/midnight-serde-rs) | `signet-midnight-serde` (crates.io) | Rust twin of the same serde byte layout: zero runtime dependencies, tested purely against the committed conformance corpus (`cargo test` needs no Node or compactc) |
