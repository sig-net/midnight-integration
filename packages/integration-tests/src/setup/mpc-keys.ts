...
/**
 * The public key the MPC network presents for a given root key: the secp256k1
 * key its EVM signing accounts derive from and its ECDSA execution
...
/**
 * Derive the MPC public key from a root key — the same call the MPC server
 * makes: the root key used directly as a secp256k1 private key, whose public
 * key both the EVM signing accounts derive from and the ECDSA attestations
