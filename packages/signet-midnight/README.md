...
- **Shared Compact modules**: the package ships its `.compact` sources, so contracts import them directly:
  ...
  ```
- **Compiled pure circuits** (`pureCircuits`): the executable reference implementation of the client-agnostic circuits (path/identity checks, the attestation-message digest, the sign-bidirectional notification packer). Off-chain code calls these compiled artefacts instead of re-porting the algorithms, so it always agrees with what the contracts prove.
...
- **Crypto helpers**: epsilon derivation and secp256k1 ECDSA attestation signing/verification.
...
4. **Verifies the execution response**: checks the attestation key, verifies the MPC's secp256k1 ECDSA attestation in-circuit over the digest of (request id, output), and consumes the request.
...
constructor(mpcPk: Secp256k1Point, signetContract: SignetNotifier) {
...
// (4) Verify the MPC's secp256k1 ECDSA-signed attestation of the remote
// execution and consume the request (double-verify protection). The signing
// public key is passed alongside the record and pinned by hash.
export circuit verifyResponse(
  ...
  assert(
    persistentHash<Secp256k1Point>(pk) == mpcPubKeyHash,
  ...
  assert(
    secp256k1EcdsaVerify(
      ...
      Secp256k1EcdsaSignature {
        r: respondBidirectional.sigR as Secp256k1Scalar,
        s: respondBidirectional.sigS as Secp256k1Scalar,
      },
      pk
    ),
...
Between the two circuits sit the off-chain steps, which this package also covers: poll the central signet contract with `SignetRequestResponseReader` (configured with both contract addresses and the ledger field position of your request index) to collect and verify the MPC's secp256k1 signature (then broadcast the signed transaction to the destination chain), and later the ECDSA-attested execution output to feed back into `verifyResponse`.
