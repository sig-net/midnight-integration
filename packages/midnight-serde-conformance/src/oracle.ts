 ...
 * @throws {Error} If the type uses a foreign field with a different FAB encoding.
 */
export function runtimeType(type: CompactType): RuntimeCompactType<unknown> {
  switch (type.kind) {
    ...
    case "secp256k1-scalar":
      throw new Error(
        "The FAB oracle uses a shifted foreign-field representation. Use compiled circuits.",
      );
    case "uint": {
      const bound =
...
/**
 * Whether the runtime FAB representation matches builtin serialisation.
 *
 * @param type - The descriptor to inspect.
 * @returns False for foreign fields or containers holding them.
 */
export function supportsFabOracle(type: CompactType): boolean {
  switch (type.kind) {
    case "secp256k1-base":
    case "secp256k1-scalar":
      return false;
    case "vector":
      return supportsFabOracle(type.element);
    case "tuple":
      return type.elements.every(supportsFabOracle);
    case "struct":
      return type.fields.every((f) => supportsFabOracle(f.type));
    default:
      return true;
  }
}
