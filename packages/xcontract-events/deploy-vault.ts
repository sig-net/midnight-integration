// Deploy entrypoint (`npm run deploy:vault`): deploys the vault (A) referencing
...
if (!tokenAddress) {
  throw new Error("XC_TOKEN_CONTRACT_ADDRESS is required (deploy the token first: npm run deploy:token)");
}
